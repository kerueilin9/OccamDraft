...
- 候選任務整併、去重與品質標記。
- `accept`、`revise`、`reobserve`、`reject` 人工審查。
- 匯出 OccamQA JSON 任務。
...
flowchart TD
    ...
    H -->|reobserve| Q["Observation Queue"]
    H -->|reject| Z["Rejected Archive"]
...
| `reobserve` | 現有證據不足 | 建立 Observation Job，再修訂相關草稿 |
| `reject` | 重複、不可執行、低價值或不應納入 | 保留於 rejected archive |
...
stateDiagram-v2
    ...
    under_review --> observation_requested: reobserve
    under_review --> rejected: reject
    revision_requested --> generated: revised version
    observation_requested --> reobserved
    reobserved --> generated: regenerate or revise
    accepted --> exported
```
...
- 每次 revise 或 reobserve 後，建立新的 `DraftVersion`。
- 每個版本保存：
  - Parent version。
  - 使用的 evidence IDs。
  - 人工意見。
...
class DraftVersion(BaseModel):
    ...
    generation_reason: Literal["initial", "revision", "reobserve"]
...
class ReviewDecision(BaseModel):
    ...
    draft_id: str
    version_id: str
    decision: Literal["accept", "revise", "reobserve", "reject"]
...
| `EvidenceGapAgent` | Draft + evidence | ReobserveRequest | 僅能提出重新觀察要求 |
...
### 8.4 Reobserve Workflow

```mermaid
flowchart LR
    H["Human reobserve request"] --> Q["Observation Job"]
    Q --> O["CLI Evidence Collector"]
    O --> E["New PageEvidence"]
    E --> R["Revision Agent"]
    R --> B["New DraftVersion"]
```
...
POST   /api/routes/{route_id}/reobserve
...
occamdraft drafts reobserve <draft_id>
```
...
- 候選數量與去重結果。
- 每個品質標記的數量。
- 人工 accept/revise/reobserve/reject 數量。
...
- Reobserve 會建立新 evidence version。
...
- 建立 Review UI。
- 實作 accept/revise/reobserve/reject 狀態機。
- 實作 Revision Agent。
- 實作 Reobserve Job。
...
- 一筆候選可經多次 revise/reobserve，最後 accept 或 reject。
- 所有版本與證據可追溯。
